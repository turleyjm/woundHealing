...
for filename in filenames:
    ...
    df = importDividingTracks(f"dat/{filename}/h2Prob{filename}.xml")
    ...
    if len(df) > 0:
        ...
        vidBinary = sm.io.imread(f"dat/{filename}/h2Prob{filename}.tif").astype(float)
        ...
        tifffile.imwrite(f"dat/{filename}/h2Binary{filename}.tif", vidBinary)

        vidHeight = sm.io.imread(f"dat/{filename}/height{filename}.tif").astype("uint8")
        ...
        binary = sm.io.imread(f"dat/{filename}/ecadBinary{filename}.tif").astype(
            "uint8"
        )
        trackBinary = binary
