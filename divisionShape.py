...
if False:
    ...
    for filename in filenames:
        ...
        dfFileShape = dfFileShape[dfFileShape["Track length"] > 24]
        df = dfDivisionTrack[dfDivisionTrack["Filename"] == filename]
        labels = list(dfFileShape["Label"])
        for label in labels:
            ori = dfFileShape["Orientation"][dfFileShape["Label"] == label].iloc[0]
            dfDiv = df[df["Label"] == label]
            oriPre = dfDiv["Orientation"][dfDiv["Division Time"] == -20].iloc[0]
            diff.append(angleDiff(ori, oriPre))
            sf.append(dfDiv["Shape Factor"][dfDiv["Division Time"] == -20].iloc[0])
...
# orientation tcj of dividing cells
if True:
    dfDivisionShape = pd.read_pickle(f"databases/dfDivisionShape{fileType}.pkl")
    dfDivisionTrack = pd.read_pickle(f"databases/dfDivisionTrack{fileType}.pkl")
    dfShape = pd.read_pickle(f"databases/dfShape{fileType}.pkl")
    ...
    for filename in filenames:
        ...
        for label in labels:
            ori = dfFileShape["Orientation"][dfFileShape["Label"] == label].iloc[0]
            dfDiv = df[df["Label"] == label]
            oriPre = dfDiv["Orientation tcj"][dfDiv["Division Time"] == -20].iloc[0]
            diff.append(angleDiff(ori, oriPre))
            sf.append(dfDiv["Shape Factor tcj"][dfDiv["Division Time"] == -20].iloc[0])
