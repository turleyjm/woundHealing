...
vidEcad = sm.io.imread(f"dat/{filename}/ecadProb{filename}.tif").astype(int)
vidH2 = sm.io.imread(f"dat/{filename}/h2Prob{filename}.tif").astype(int)

vidEcad[vidWound == 255] = 0

vidEcad = np.asarray(vidEcad, "uint8")
tifffile.imwrite(f"dat/{filename}/ecadWoundmask{filename}.tif", vidEcad)
