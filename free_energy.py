...
num_frames = 14
num_videos = int(n / 14)
TrS = np.zeros((num_frames, num_videos))
...
for video in range(num_videos):
    ...
    for frame in range(num_frames):
        ...
        TrS[frame, video] = 2 * c
        dQr12[frame, video] = cell.mean(dQr1 ** 2)
        dQr22[frame, video] = cell.mean(dQr2 ** 2)
...
for frame in range(num_frames):  # finds means and ver over the videos
    ...
    sigma2 = sum(
        TrS[frame, 0 : num_videos + 1] ** 2 * count[frame, 0 : num_videos + 1]
    ) / sum(count[frame, 0 : num_videos + 1])
    err_TrS.append((sigma2 / sum(count[frame, 0 : num_videos + 1])) ** 0.5)

    TrS[frame] = sum(
        TrS[frame, 0 : num_videos + 1] * count[frame, 0 : num_videos + 1]
    ) / sum(count[frame, 0 : num_videos + 1])
...
fig.savefig(
    "results/free_energy/" + FIGDIR + f"_dQr12_and_dQr22", dpi=300, transparent=True,
)
plt.close("all")

xs = np.array(list(x), dtype=np.float64)
ys = np.array(TrS, dtype=np.float64)
(m, d) = best_fit_slope_and_intercept(xs, ys)

fig = plt.figure(1, figsize=(9, 8))
plt.gcf().subplots_adjust(left=0.2)
plt.errorbar(x, TrS, yerr=err_TrS, fmt=".")
plt.plot([0, 13], [d, 13 * m + d], "k-", lw=2)
# plt.title(f"m = {m}, d = {d}")
plt.xlabel("Time")
# plt.ylabel(r"$\aver{\norm{\bmQ}^2}$")
fig.savefig(
    "results/free_energy/" + FIGDIR + f"_TrS", dpi=300, transparent=True,
)
plt.close("all")
