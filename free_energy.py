...
if False:
    ...
    for frame in range(num_frames):  # finds means and var over the videos
        ...
        err_dQ1 += sum(
            (dQr12[frame, 0 : num_videos + 1] - dQ1) ** 2
            * count[frame, 0 : num_videos + 1]
        )
        err_dQ2 += sum(
            (dQr22[frame, 0 : num_videos + 1] - dQ2) ** 2
            * count[frame, 0 : num_videos + 1]
        )
    ...
    if abs(kBb - kBb_up) > abs(kBb - kBb_down):
        ...
        print(
            r"$\frac{k_B}{b_0} =$" + f" {kBb}" + r" $\pm$ " + f"{abs(kBb - kBb_down)}"
        )
    ...
    if abs(kBa - kBa_up) > abs(kBa - kBa_down):
        ...
        print(
            r"$\frac{k_B}{a_0} =$" + f" {kBa}" + r" $\pm$ " + f"{abs(kBa - kBa_down)}"
        )

# correlation of Q_0 and Q^hat
