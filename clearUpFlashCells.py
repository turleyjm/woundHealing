...
for filename in filenames:
    ...
    for t in range(1, T - 1):
        ...
        for colour in uniqueFrame[t]:
            ...
            if preFlash and postFlash:
                ...
                # vid[t][np.all((vid[t] - colour) == 0, axis=2)] = np.array([0, 0, 0])
    ...
    for t, colour in flashs:
        ...
        contourReal = sm.measure.find_contours(vidLabel[t] == realLabel, level=0)[0]
        contourFlash = sm.measure.find_contours(vidLabel[t] == flashLabel, level=0)[0]
        ...
        if realLabel != flashLabel:
            ...
            for con in contourFlash:
                if len(contourReal[np.all((contourReal - con) == 0, axis=1)]) == 1:
                    overlap.append(
