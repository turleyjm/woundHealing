...
class STARMA:
    ...
    def __str__(self):
        if self._model is not None:
            ...
            return (
                "Object of class STARMA/STARIMA "
                "\n\t AR-Orders: %s "
                "\n\t MA-Orders: %s " % (self._p, self._q)
            )
    ...
    def _fit_model(self, ts_matrix):
        ...
        # first iteration
        eps = ts_matrix.copy()
        self._model = utils.kalmanfilter_estimation(
            ts_matrix,
            self._wa_matrices,
            eps,
            self._get_ar_matrix(),
        ...
        if self._q > 0:
            ...
            while self._iter > count:
                eps[0 : self._max_tlag] = utils.residuals_estimation(
                    ts_matrix[0 : self._max_tlag],
                    self._wa_matrices,
                    self._model["phi"],
                    self._model["theta"],
                )
        ...
        self._model["residuals"] = utils.residuals_estimation(
            ts_matrix, self._wa_matrices, self._model["phi"], self._model["theta"]
        )
        self._model["sigma2"] = np.trace(self._model["sigma2VarianceMatrix"]) / len(
            self._model["sigma2VarianceMatrix"]
        )
        self._model["llh"] = utils.loglikelihood(self._model) + np.log(
            ts_matrix.size * ((self._get_total_parameter()) * len(self._wa_matrices))
        )
        self._model["bic"] = (
            self._get_total_parameter() * np.log(ts_matrix.size)
            - 2 * self._model["llh"]
        )
    ...
    def _p_value(self, t_value):
        ...
        df = (self._ts_matrix.size) - (
            self._get_total_parameter() * len(self._wa_matrices)
        )
    ...
    def print_results(self):
        ...
        if self._model != 0:
            ...
            print(
                "standard error is estimated as:\t\t %s"
                % np.sqrt(self._model["sigma2"])
            )
