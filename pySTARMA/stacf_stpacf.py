...
class Stacf:
    ...
    def _st_cov(ts_matrix, w1, w2, t_lag):
        ...
        gamma = 0
        power_wn = np.array(w2.T.dot(w1))

        for t in range(0, max_lags):
            gamma += (
                power_wn.dot(ts_matrix[[t + t_lag]].T.dot(ts_matrix[[t]]))
            ).trace()
    ...
    def _st_vec(self):
        ...
        for t in range(1, self._t_lags + 1):
            for s, wm in enumerate(self._wa_matrices):
                st_vec[(t - 1) * self._s_lags + s] = self._st_cov(
                    self._ts_matrix, wm, self._wa_matrices[0], t
                )
        return st_vec
    ...
    def _st_pacf(self):
        ...
        for t in range(0, t_lag):
            for s in range(0, s_lag):
                index = t * s_lag + s
                sol = np.linalg.solve(
                    YWmat[: index + 1, : index + 1], YWvec[: index + 1]
                )
