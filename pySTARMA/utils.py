...
def kalmanfilter_estimation(
    ts_matrix, wa_matrices, residuals, ar_matrix, ma_matrix, p_lag, q_lag, max_t_lag
):
    """
    ...
    p = 100000.0 * np.eye(dim, dim)
    sigma2 = (1 / 100000.0) * np.eye(ts_matrix.shape[1], ts_matrix.shape[1])
    ...
    for t in range(max_t_lag, ts_matrix.shape[0]):
        ...
        sigma2 = (sigma2 * (t + 1 - max_t_lag) + nu.T * nu) / (
            t + 2 - max_t_lag
        )  # 2.30 Cipra & Motykova 1987
    ...
    for t in range(ts_matrix.shape[0]):
        ...
        for t_lag in range(t_lim):
            for slag in range(0, len(phi[0])):
                ...
                residuals[[t]] -= (ts_matrix[[t - t_lag - 1]] * phi[t_lag, slag]).dot(
                    weights.T
                )
        ...
        for t_lag in range(t_lim):
            for slag in range(0, len(theta[0])):
                weights = wa_matrices[slag]
                residuals[[t]] -= (residuals[[t - t_lag - 1]] * theta[t_lag, slag]).dot(
                    weights.T
                )
    return residuals
...
def set_stationary(ts_matrix, lags):
    ...
    return time_series_matrix.to_numpy()
